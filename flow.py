...
class Flow:
    ...
    def __init__(self, driver_dependecy: dict, routes: dict, service_acc_token_fp: str):
        ...
        Commands.sigkill_appium_process(self.port).release_port_adb(self.udid).kill_app(
            self.udid, self.pkg_name
        ...
        self.g_sheet = GoogleSheet(
            service_acc_token_fp,
            driver_dependecy["gsheet_id"],
            driver_dependecy["sheet_name"],
        )

        self.server = Server().start_server(self.port, self.udid)

        # before selenium remote connection

        self.driver = Device(
            driver_dependecy["desired_caps"], driver_dependecy["host"]
        ).get_driver()
    ...
    def start(self):
        root_view, modal_view, search_modal_view, slider_modal_view = self.phase_one()
        ...
        while True:
            try:
                self.iteration(
                    root_view, search_modal_view, modal_view, slider_modal_view
                )
                ...
                Commands.kill_app(self.udid, self.pkg_name).sigkill_appium_process(
                    self.port
                ).release_port_adb(self.udid).clean_appium_pkgs(self.udid)

                sys.exit(1)

    def iteration(self, root_view, search_modal_view, modal_view, slider_modal_view):
    ...
    def phase_four(
        self, search_modal_view: SearchModalView, modal_obj, root_view_obj, destination
    ):
        search_modal_view.select_destination_route(
            destination, modal_obj, root_view_obj
        )
    ...
    def phase_six(self, slider_modal_view: SliderModal):
        return (
            (slider_modal_view.get_price("Economy"), slider_modal_view.get_eta()),
            (slider_modal_view.get_price("Comfort"), slider_modal_view.get_eta()),
            (slider_modal_view.get_price("Comfort+"), slider_modal_view.get_eta()),
        )
    ...
    def _reduce_routes_hash(self, last_checked_km):
        ...
        km_keys = list(map(lambda x: str(x), list(self.routes.keys())))
