...
class Commands:
    ...
    def launch_app(uuid, android_pkg_name):
        ...
        os.system(
            f"adb -s {uuid} shell monkey -p {android_pkg_name} -c android.intent.category.LAUNCHER 1"
        )
        time.sleep(12)
    ...
    def is_device_online(uuid):
        if (
            ...
            not in subprocess.check_output(
                f"adb -s {uuid} devices", shell=True
            ).decode()
        ):
    ...
    def clean_appium_pkgs(uuid):
        for pkg_name in (
            ...
            "io.appium.uiautomator2.server.test" "io.appium.unlock",
            "io.appium.settings",
        ):
